...
class AboutPageIT {
    ...
    void allContentPresent() {
        ...
        driver.navigate().to("http://localhost:" + port +"/about");

        var H1 = driver.findElements(By.id("about-title"));
        assertEquals(1, H1.size());

        var nav = driver.findElements(By.cssSelector("nav"));
        assertEquals(1, nav.size());

        var descriptions = driver.findElements(By.cssSelector(".description"));
